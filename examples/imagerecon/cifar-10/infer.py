#!/usr/bin/env python3
"""Inference script for Image Reconstruction on CIFAR-10 - reconstruct individual images."""
...
def load_and_preprocess_image(image_path: str, mean: tuple, std: tuple) -> torch.Tensor:
    ...
    if image_path == "random":
        # Generate random CIFAR-10 like image
...
def create_visualization(original, masked, reconstructed, **kwargs):
    ...
    full_output_path = viz_dir / output_path
    plt.savefig(full_output_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved reconstruction to {full_output_path}")

    return viz_dir


def save_individual_images(original, masked, reconstructed, viz_dir, output_base):
    """Save individual images."""
    transforms.ToPILImage()(original[0]).save(viz_dir / f"{output_base}_original.png")
    transforms.ToPILImage()(masked[0]).save(viz_dir / f"{output_base}_masked.png")
    transforms.ToPILImage()(reconstructed[0]).save(
        viz_dir / f"{output_base}_reconstructed.png"
    )
    print(f"Saved individual images to {viz_dir}")
...
def main(cfg: DictConfig) -> None:
    ...
    save_individual_images(original, masked, reconstructed, viz_dir, output_base)
